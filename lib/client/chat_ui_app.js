...
function weatherMetric(label, value) {
  return value ? createElement('p', 'weather-metric', `${label} ${value}`) : null;
}
...
class TravelStayApp {
  constructor(root) {
    ...
    this.scrollFollow = createScrollFollowState();
    this.scrollRaf = null;
      ...
      <main class="page-shell">
        <section class="hero-panel">
          <div class="hero-copy">
            ...
            <p>输入自然语言需求，系统会实时生成结构化住宿建议，帮助你比较酒店、路线和出行取舍。</p>
          </div>
        ...
        <section class="demo-panel hidden" id="demo-panel" aria-label="流式组件验证">
          <span>演示信息</span>
          <p>当前页面采用组件级流式渲染：推荐、理由、路线和天气模块会随结果逐步到达而更新。酒店数据为课程演示 Mock 数据，天气数据来自 Open-Meteo。</p>
        ...
        <div class="result-stream-end" id="result-stream-end" aria-hidden="true"></div>
        <button class="new-results-button hidden" type="button" id="new-results-button" aria-label="查看最新结果">有新结果 ↓</button>
      </main>
  ...
  collectNodes() {
    ...
    this.nodes.streamEnd = $('#result-stream-end', this.root);
    this.nodes.newResults = $('#new-results-button', this.root);
  ...
  bindEvents() {
    ...
    this.nodes.newResults.addEventListener('click', () => this.followLatestResult(true));
    window.addEventListener('scroll', () => this.handleWindowScroll(), { passive: true });
    window.addEventListener('wheel', (event) => this.handleIntentionalScroll(event.deltaY), { passive: true });
    window.addEventListener('touchmove', () => this.handleIntentionalScroll(-1), { passive: true });
    window.addEventListener('keydown', (event) => {
      if (['PageUp', 'ArrowUp', 'Home', 'Space'].includes(event.code)) this.handleIntentionalScroll(-1);
      if (['PageDown', 'End'].includes(event.code)) window.requestAnimationFrame(() => this.handleWindowScroll());
    });
  ...
  scrollMetrics() {
    ...
    return {
      scrollY: window.scrollY || doc.scrollTop || body.scrollTop || 0,
      viewportHeight: window.innerHeight || doc.clientHeight || 0,
      scrollHeight: Math.max(doc.scrollHeight, body.scrollHeight, doc.offsetHeight, body.offsetHeight),
    };
  }

  updateNewResultsButton() {
    this.nodes.newResults.classList.toggle('hidden', !this.scrollFollow.hasPendingNewResults);
  }

  handleWindowScroll() {
    if (this.suppressScrollHandling) return;
    this.scrollFollow = updateScrollPosition(this.scrollFollow, this.scrollMetrics());
    this.updateNewResultsButton();
  }

  handleIntentionalScroll(deltaY) {
    if (deltaY >= 0) return;
    this.suppressScrollHandling = false;
    window.requestAnimationFrame(() => this.handleWindowScroll());
  }

  followLatestResult(fromButton = false) {
    this.scrollFollow = markFollowed(this.scrollFollow);
    this.updateNewResultsButton();
    if (!this.nodes.streamEnd) return;
    this.suppressScrollHandling = true;
    this.nodes.streamEnd.scrollIntoView({
      behavior: this.scrollBehavior(),
      block: 'end',
    });
    window.setTimeout(() => {
      this.suppressScrollHandling = false;
      if (fromButton) this.handleWindowScroll();
    }, this.scrollBehavior() === 'smooth' ? 520 : 0);
  }

  handlePostRenderFollow(view) {
    const signature = visibleContentSignature(view);
    if (!signature) return;
    if (this.scrollRaf) window.cancelAnimationFrame(this.scrollRaf);
    this.scrollRaf = window.requestAnimationFrame(() => {
      this.scrollRaf = null;
      const result = handleVisibleContentChange(this.scrollFollow, this.scrollMetrics(), signature);
      this.scrollFollow = result.state;
      this.updateNewResultsButton();
      if (result.shouldFollow) this.followLatestResult(false);
    });
  }
  ...
  resetForNewResponse() {
    this.state = createInitialChatState();
    this.scrollFollow = resetScrollFollowState(this.scrollFollow);
    this.resetResultDom();
    this.render(this.state);
    this.updateNewResultsButton();
  }
  ...
  resetSession() {
    ...
    this.nodes.textarea.focus();
    this.nodes.form.scrollIntoView({ behavior: this.scrollBehavior(), block: 'start' });
  }
  ...
  render(state) {
    ...
    this.renderProtocolErrors(view.protocolErrorNotice);
    this.handlePostRenderFollow(view);
  }
  ...
  renderProtocolErrors(notice) {
    if (!notice) return;
    this.nodes.error.classList.remove('hidden');
    clear(this.nodes.error);
    this.nodes.error.append(
      createElement('strong', '', notice.title),
      createElement('p', '', notice.message),
    );
    if (this.debugMode && Array.isArray(notice.diagnostics) && notice.diagnostics.length > 0) {
      const list = createElement('ul', 'debug-error-list');
      notice.diagnostics.forEach((item) => {
        list.appendChild(createElement('li', '', `${item.code}: ${item.message}`));
      });
      this.nodes.error.appendChild(list);
    }
  }
  ...
  renderReasons(reasons) {
    ...
    if (!this.nodes.reasons.querySelector('.section-heading')) {
      this.nodes.reasons.appendChild(sectionTitle('为什么推荐它', '综合预算、位置、路线和旅行偏好形成的推荐理由。'));
  ...
  renderRoute(routeView) {
    ...
    section.appendChild(sectionTitle('交通与路线', routeView.description));
    if (routeView.valid === false) {
      section.appendChild(createElement('p', 'muted', routeView.unavailableText || '本次暂未获得可比较的路线数据。'));
    } else {
  ...
  renderWeather(weather) {
    ...
    fact.appendChild(createElement('strong', '', weather.statusLabel));
    if (weather.summary) fact.appendChild(createElement('p', '', weather.summary));
    if (weather.sourceLabel) fact.appendChild(createElement('p', 'muted', weather.sourceLabel));
    section.appendChild(fact);
    if (Array.isArray(weather.dailyForecasts) && weather.dailyForecasts.length > 0) {
      const daily = createElement('div', 'weather-daily-grid');
      weather.dailyForecasts.forEach((day) => {
        const card = createElement('article', 'weather-day-card');
        card.appendChild(createElement('h3', '', day.dateLabel || day.date));
        [
          weatherMetric('最高', formatNumber(day.tempMaxC, '°C') + (typeof day.tempMinC === 'number' ? ` · 最低 ${formatNumber(day.tempMinC, '°C')}` : '')),
          weatherMetric('降雨概率', formatNumber(day.precipitationProbabilityPct, '%') + (typeof day.precipitationSumMm === 'number' ? ` · 降水量 ${formatNumber(day.precipitationSumMm, ' mm')}` : '')),
          weatherMetric('最大风速', formatNumber(day.windSpeedMaxKmh, ' km/h')),
        ].filter(Boolean).forEach((metric) => card.appendChild(metric));
        daily.appendChild(card);
      });
      section.appendChild(daily);
    }
    if (weather.riskLabel || (Array.isArray(weather.riskMessages) && weather.riskMessages.length > 0)) {
      const risk = createElement('div', 'advice-block weather-risk');
      risk.appendChild(createElement('strong', '', '天气提醒'));
      if (weather.riskLabel) risk.appendChild(createElement('p', '', weather.riskLabel));
      if (Array.isArray(weather.riskMessages) && weather.riskMessages.length > 0) {
        const list = createElement('ul', 'weather-risk-list');
        weather.riskMessages.forEach((message) => list.appendChild(createElement('li', '', message)));
