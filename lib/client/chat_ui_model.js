...
function weatherCoverageLabel(coverageMode, available) {
  if (coverageMode === 'full') return '旅行日期范围内天气数据完整';
  if (coverageMode === 'partial') return '部分旅行日期可提供天气信息';
  if (coverageMode === 'unavailable') return '当前旅行日期暂不在可用天气范围内';
  return available ? '天气信息可用于本次出行判断' : '当前天气信息暂不可用';
}

function weatherSourceLabel(source) {
  if (source === 'open_meteo') return '天气数据：Open-Meteo';
  if (source === 'degraded_context') return '天气信息：降级说明';
  return source ? '天气信息来源已记录' : '';
}
...
function riskLevelLabel(level) {
  if (level === 'high') return '降雨或天气影响风险较高';
  if (level === 'medium') return '需要关注天气变化';
  if (level === 'low') return '当前天气风险相对较低';
  return '';
}
...
function normalizeDailyWeather(item) {
  ...
  return {
    ...
    tempMaxC: numberOrNull(item.temp_max_c ?? item.temperature_2m_max),
    tempMinC: numberOrNull(item.temp_min_c ?? item.temperature_2m_min),
    precipitationProbabilityPct: numberOrNull(item.precip_probability_max_pct ?? item.precipitation_probability_max),
    precipitationSumMm: numberOrNull(item.precipitation_sum_mm ?? item.precipitation_sum),
    windSpeedMaxKmh: numberOrNull(item.wind_speed_max_kmh ?? item.wind_speed_10m_max),
  };
}

function parseWeatherContext(context) {
  if (typeof context !== 'string' || context.trim() === '') {
    return { dailyForecasts: [], riskLevelLabel: '', riskMessages: [] };
  }

  const dailyForecasts = [];
  const riskMessages = [];
  let riskLevel = '';
  let inRiskMessages = false;
  const lines = context.split(/\r?\n/).map((line) => line.trim()).filter(Boolean);

  for (const line of lines) {
    const dailyMatch = /^(\d{4}-\d{2}-\d{2})[：:]\s*最高\s*([\d.]+)°?C[，,]\s*最低\s*([\d.]+)°?C[，,]\s*最大降雨概率\s*([\d.]+)%[，,]\s*降水量\s*([\d.]+)\s*mm[，,]\s*最大风速\s*([\d.]+)\s*km\/h[。.]?$/.exec(line);
    if (dailyMatch) {
      dailyForecasts.push(normalizeDailyWeather({
        date: dailyMatch[1],
        temp_max_c: dailyMatch[2],
        temp_min_c: dailyMatch[3],
        precip_probability_max_pct: dailyMatch[4],
        precipitation_sum_mm: dailyMatch[5],
        wind_speed_max_kmh: dailyMatch[6],
      }));
      inRiskMessages = false;
      continue;
    }

    const riskMatch = /^天气风险等级[：:]\s*([a-zA-Z_]+)$/.exec(line);
    if (riskMatch) {
    ...
    if (inRiskMessages && line.startsWith('-')) {
      const message = line.replace(/^-\s*/, '').trim();
      if (message) riskMessages.push(message);
    }
  }

  return {
    dailyForecasts: dailyForecasts.filter(Boolean),
    riskLevelLabel: riskLevelLabel(riskLevel),
    riskMessages,
  };
}

function structuredDailyWeather(weather) {
  if (!Array.isArray(weather?.daily)) return [];
  return weather.daily.map(normalizeDailyWeather).filter(Boolean);
}
...
function buildRouteView(state) {
  ...
  if (routeMatrix.valid === false) {
    return {
      ...
      description: '比较候选酒店前往主要活动地点的车程与距离。',
      unavailableText: '本次暂未获得可比较的路线数据。',
    };
  }
...
function buildWeatherView(state) {
  ...
  const structuredDaily = structuredDailyWeather(weather);
  const parsedContext = structuredDaily.length > 0
    ? { dailyForecasts: structuredDaily, riskLevelLabel: riskLevelLabel(weather?.risk_level), riskMessages: Array.isArray(weather?.risk_messages) ? weather.risk_messages : [] }
    : parseWeatherContext(weather?.context || '');
  const unavailableText = '当前旅行日期暂不在可用天气预报范围内，住宿建议仍基于预算、位置、路线和旅行偏好生成。';
  return {
    available,
    statusLabel: weatherCoverageLabel(weather?.coverage_mode || '', available),
    dailyForecasts: available ? parsedContext.dailyForecasts : [],
    riskLabel: available ? parsedContext.riskLevelLabel : '',
    riskMessages: available ? parsedContext.riskMessages : [],
    summary: available
      ? (parsedContext.dailyForecasts.length > 0 ? '' : '本次天气信息可用，建议结合天气建议安排户外活动。')
      : unavailableText,
    sourceLabel: weatherSourceLabel(weather?.source || ''),
    advice: state.weatherAdvice?.content || '',
  };
}

function buildProtocolErrorNotice(state) {
  const errors = Array.isArray(state.protocolErrors) ? state.protocolErrors : [];
  if (errors.length === 0) return null;
  return {
    title: '内容生成提示',
    message: '部分内容生成过程中出现异常，已保留此前成功生成的结果。',
...
function buildChatViewModel(state) {
  return {
    ...
    protocolErrorNotice: buildProtocolErrorNotice(state),
...
module.exports = {
  ...
  buildRouteView,
  buildWeatherView,
  buildProtocolErrorNotice,
  createIndexes,
  dimensionPresentation,
  parseWeatherContext,
  weatherCoverageLabel,
};
